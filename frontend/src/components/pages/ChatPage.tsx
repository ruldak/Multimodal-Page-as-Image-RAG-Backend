...
export function ChatPage() {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-[calc(100vh-8rem)] gap-4 lg:gap-6 animate-fade-in">
      {/* Session List */}
      <Card className={cn(
        "flex flex-col overflow-hidden transition-all duration-300 ease-in-out shrink-0",
        showSessionList ? "lg:w-72 w-full h-48 lg:h-auto" : "lg:w-0 w-full h-0 lg:h-auto overflow-hidden opacity-0 lg:opacity-100"
      )}>
      ...
      <Card className="flex-1 flex flex-col overflow-hidden min-w-0">
        {!activeSession ? (
          ...
          <>
            ...
            <div className="flex-1 overflow-auto p-4 md:p-6 space-y-4 md:space-y-6 bg-slate-50/30 min-w-0">
              {activeSession.messages.map((msg) => (
                <div key={msg.id} className={cn("flex gap-3 md:gap-4", msg.role === "user" ? "justify-end" : "justify-start")}>
                  <div className={cn("flex gap-2 md:gap-3 max-w-[90%] md:max-w-[80%]", msg.role === "user" && "flex-row-reverse")}>
                    <div className={cn(
                    ...
                    <div className={cn("space-y-2 min-w-0", msg.role === "user" && "items-end")}>
                      <div className={cn(
                        ...
                        <div className="flex flex-wrap gap-2">
                          {msg.sources.map((src, idx) => (
                            <div key={idx} className="group relative">
                              <div className="w-14 h-18 md:w-16 md:h-20 rounded-lg border border-slate-200 overflow-hidden bg-white hover:border-primary-300 transition-colors cursor-pointer">
                                <img
              ...
              {stream.isStreaming && (
                <div className="flex gap-3 md:gap-4 justify-start">
                  <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-primary-100 text-primary-700 flex items-center justify-center shrink-0 mt-0.5">
                    ...
                    {stream.sources.length > 0 && (
                      <div className="flex flex-wrap gap-2">
                        {stream.sources.map((src, idx) => (
                          <div key={idx} className="w-14 h-18 md:w-16 md:h-20 rounded-lg border border-slate-200 overflow-hidden bg-white">
            ...
            <div className="p-3 md:p-4 border-t border-slate-200 bg-white shrink-0">
              <div className="flex gap-2 md:gap-3">
                <input
                  ...
                  className="flex-1 h-10 md:h-11 px-3 md:px-4 rounded-lg border border-slate-200 bg-slate-50 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all"
                />
      ...
      {showNewChat && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 md:p-8 animate-fade-in">
          <Card className="w-full max-w-md p-4 md:p-6 max-h-[90vh] overflow-auto">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="space-y-2 max-h-48 overflow-auto border border-slate-200 rounded-lg p-2">
                  <button
